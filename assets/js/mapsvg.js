    ...
    this.methods = {
      ...
      mapAdjustStrokes: function() {
        _data.R.forEach(function(region) {
          if (region.default_attr && region.default_attr['stroke-width'])
            region.attr({
              'stroke-width': region.default_attr['stroke-width'] / _data.scale
            });
        });
      },
      // ZOOM
      ...
      marksAdjustPosition: function(mark) {
        ...
        for (var m = 0; m < _data.RMarks.items.length; m++) {
          ...
          _data.RMarks.items[m].attr({
            width: w / _data.scale,
            height: h / _data.scale
          }).transform('t' + dx + ',' + dy);
        }
      ...
      unhighlightRegion: function(id) {
        var region = _data.R.getById(id);
        if (region.disabled || (_data.options.multiSelect && $.inArray(id, _data.selected_id) >= 0) || _data.selected_id == id) return false;
      ...
      highlightRegion: function(id) {
        var region = _data.R.getById(id);
        if (_data.isPanning || region.disabled || (_data.options.multiSelect && $.inArray(id, _data.selected_id) >= 0) || _data.selected_id == id)
          return false;
      ...
      renderSVGText: function(textObj) {
        ...
        tspans.each(function(i, tspan) {
          var t = _this.renderSVGTspan($(tspan), {
            x: x,
            y: y
          });
      ...
      renderSVGTspan: function(textObj, parent) {
        ...
        var rObj = _data.R.text(x, y, text).attr({
          'text-anchor': 'start'
        }).toFront();
      ...
      setZoom: function(on) {
        if (on) {
          ...
          if (_data.options.zoomButtons.show) {
            ...
            buttons.append(btnZoomOut).append(btnZoomIn).css({
              position: 'absolute',
              top: '15px',
              width: '16px',
              cursor: 'pointer'
          ...
          $.ajax({
            ...
            success: function(xmlData) {
                ...
                if (_data.options.panBackground)
                  _data.background = _data.R.rect(_data.svgDefault.viewBox[0], _data.svgDefault.viewBox[1], _data.svgDefault.viewBox[2], _data.svgDefault.viewBox[3]).attr({
